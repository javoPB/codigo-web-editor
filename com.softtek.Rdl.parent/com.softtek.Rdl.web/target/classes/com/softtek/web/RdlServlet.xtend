...
/**/
	override protected service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {		
		//val String pathDirectoryProject = "C:\\Users\\javier.perezb\\Desktop\\temporal\\temporal222";
		//val String pathLogFile = "C:\\Users\\javier.perezb\\Desktop\\temporal\\log.txt";
		val String pathDirectoryProject = File.separator+"src"+File.separator+"rdl-directory";
		val String pathLogFile = File.separator+"src"+File.separator+"logs"+File.separator+"log.txt";							
		...
		try {
			...
			if( !action.isNullOrEmpty && action.equals('clone') ){
				...
				if( isNewRdl.equals("true") ){
					val boolean existDirectory = new UtilGIT().existDirectory(pathDirectoryProject + "\\Rdl\\");
					if( !existDirectory ){
						new UtilGIT().createDirectory(pathDirectoryProject + "\\Rdl\\");
					}
										
					val String treeView = new UtilGIT().saveFile(pathDirectoryProject + "\\Rdl\\" + pathFile, data, pathDirectoryProject);
				}else{
					val String treeView = new UtilGIT().saveFile(pathFile, data, pathDirectoryProject);
				}
				*/
